...
qiime taxa filter-table \
  --i-table table-$name""_feature.qza \
  --i-taxonomy taxonomy-$name"".qza \
  --p-mode exact \
  --p-exclude "k__Bacteria; p__Firmicutes; c__Bacilli; o__Lactobacillales; f__Lactobacillaceae" \
  --o-filtered-table table-$name""_feature2.qza  
...
bash ../16sscripts/standard_qiime3.sh $name""_noW $name All 200 $mapper "lab IN ('UCLA','Stanford','Bellen','Pallanck')"

bash ../16sscripts/standard_qiime3.sh $name""_noOther $name pinks 164 $mapper "genotype IN ('PG (Control)','pink15')"

bash ../16sscripts/standard_qiime3.sh $name""_noOther $name pinks_noControl 164 $mapper "genotype IN ('pink15')"

bash ../16sscripts/standard_qiime3.sh $name""_noOther $name Bloom_pink 164 $mapper "lab = 'Bloomington' AND genotype = 'pink15'"
